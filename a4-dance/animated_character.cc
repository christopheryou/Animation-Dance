                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
	...
	not just the parent_transform but also the local rotation of the bone due to the current pose.
        ...
        Think of the vertices that make up the geometry of each bone as being defined in "bone space",
		where the joint that the bone rotates around is located at the origin and the bone extends in the direction 
		and length specified by the skeleton. (See Skeleton::BoneDirectionAndLength()).
     
        To determine which matrices need to be composed to create the current transformation matrix and the order 
		to multiply them together, think about what needs to happen to each vertex of a cylinder defined in "bone space" 
		...
		guaranteed to line up perfectly with the bone's X,Y,Z axes.  The bone's rotation axes are a property of the skeleton
		-- they are set for each skeleton and do not change for each pose.  You can access a matrix that transforms from 
		"bone space" to "rotation axis space" from the skeleton_ member variable.

        Second, now that the vertices are in the bone's "rotation axis space", the rotation from the character's 
		current pose can be applied.  The current pose is stored in the pose_ member variable.

        Third, with the rotations applied relative to the appropriate rotation axes, the vertices must now be 
		transformed back into regular "bone space".  At this point, the bone should be properly rotated based upon the current pose, 
    ...
    */
	...
	//Vector ??
	Vector3 vec = skeleton_.BoneDirectionAndLength(bone_name);
	Matrix4 RotAxesSpace = skeleton_.BoneSpaceToRotAxesSpace(bone_name);
	//Matrix4 currentPose = pose_.root_rotation();
	Matrix4 poseRot = pose_.JointRotation(bone_name);
	Matrix4 BoneSpace = skeleton_.RotAxesSpaceToBoneSpace(bone_name);

	//Current Order?
    Matrix4 ctm = parent_transform * BoneSpace * poseRot * RotAxesSpace;
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
		Color c = Color(0, 0, 0);
		quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, c, Point3(0, 0, 0), Point3(0, 0, 0) + vec, 0.01f);
    }
    ...
    if (bone_name == "lowerback") {
		Color c = Color(.702, 0, 0);
		Matrix4 S = Matrix4::Scale(Vector3(0.3, 0.15, 0.15));
		Matrix4 T = Matrix4::Translation(Vector3(-0.8, -0.40, 0));
		Matrix4 R = Matrix4::RotationZ(GfxMath::ToRadians(45.0f));
		Matrix4 R2 = Matrix4::RotationY(GfxMath::ToRadians(-90.0f));
		quick_shapes_.DrawSphere(ctm * R2 * R * S * T, view_matrix, proj_matrix, c);
    }
    if (bone_name == "upperback") {
		Matrix4 S = Matrix4::Scale(Vector3(0.15, 0.15, 0.15));
		Matrix4 T = Matrix4::Translation(Vector3(0, -0.15 / 3, 0));
		Color c = Color(.702, 0, 0);
		quick_shapes_.DrawSphere(ctm * T * S, view_matrix, proj_matrix, c);
    }
	if (bone_name == "thorax") {
		Matrix4 S = Matrix4::Scale(Vector3(0.15, 0.15, 0.15));
		Color c = Color(.702, 0, 0);
		quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, c);
	}
    ...
    if (bone_name == "head") {
		//quick_shapes_.DrawAxes(ctm * Matrix4::Scale(Vector3(0.5,0.5,0.5)), view_matrix, proj_matrix);

		//The Head Itself
		Matrix4 S = Matrix4::Scale(Vector3(0.125, 0.2, 0.15));
		Matrix4 T = Matrix4::Translation(Vector3(0, 0.1, 0.1));
		Matrix4 R = Matrix4::RotationX(GfxMath::ToRadians(-45.0f));
		Color c = Color(.702, 0, 0);
		quick_shapes_.DrawSphere(ctm * R * T * S, view_matrix, proj_matrix, c);

		//Right Eye
		S = Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
		T = Matrix4::Translation(Vector3(0.3 / 4.0f, 0.15 * 1.75f, 0.275 / 4.0f));
		c = Color(0, 0, 0);
		quick_shapes_.DrawSphere(ctm * T * S, view_matrix, proj_matrix, c);

		//Left Eye
		S = Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
		T = Matrix4::Translation(Vector3(-0.3 / 4.0f, 0.15 * 1.7f, 0.275 / 4.0f));
		c = Color(0, 0, 0);
		quick_shapes_.DrawSphere(ctm * T * S, view_matrix, proj_matrix, c);

		//Nose Thing
		S = Matrix4::Scale(Vector3(0.04, 0.04, 0.02));
		T = Matrix4::Translation(Vector3(0, 0.07, 0.70 / 4.0f));
		R = Matrix4::RotationX(GfxMath::ToRadians(20.0f));
		quick_shapes_.DrawSphere(ctm * T * R * S, view_matrix, proj_matrix, c);

		//Right Antenna
		S = Matrix4::Scale(Vector3(0.03, 0.12, 0.03));
		T = Matrix4::Translation(Vector3(0.3 / 4.0f, 0.14 * 2.5, -0.05));
		R = Matrix4::RotationZ(GfxMath::ToRadians(-20.0f));
		quick_shapes_.DrawCone(ctm * T * R * S, view_matrix, proj_matrix, c);
		...
		//Right Upper Antenna
		S = Matrix4::Scale(Vector3(0.03, 0.10, 0.03));
		T = Matrix4::Translation(Vector3(0.34 / 4.0f, 0.175 * 2.5, 0.02));
		R = Matrix4::RotationX(GfxMath::ToRadians(90));
		quick_shapes_.DrawCone(ctm * T * R * S, view_matrix, proj_matrix, c);
    ...
    Matrix4 child_root_transform = ctm * Matrix4::Translation(vec);
